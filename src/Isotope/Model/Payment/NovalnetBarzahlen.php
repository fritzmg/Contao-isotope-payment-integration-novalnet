...
class NovalnetBarzahlen extends Postsale implements IsotopePayment
{
    /**
     * @var NovalnetHelper
     */
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        $data['transaction']['test_mode'] = !empty($this->novalnetbarzahlenTestMode) ? $this->novalnetbarzahlenTestMode : '0';
        ...
        $slipExpiryDate = !empty($this->novalnetbarzahlenSlipExpiryDate) ? $this->novalnetbarzahlenSlipExpiryDate : 14;
        $data['transaction']['due_dates'] = array(
                    'CASHPAYMENT' => date('Y-m-d', strtotime('+'.$slipExpiryDate.' days'))
                );
