...
class NovalnetInvoice extends Postsale implements IsotopePayment
{
    /**
     * @var NovalnetHelper
     */
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        $data['transaction']['test_mode'] = !empty($this->novalnetinvoiceTestMode) ? $this->novalnetinvoiceTestMode : '0';
        ...
        if (!empty($this->novalnetinvoiceDueDate) && is_numeric($this->novalnetinvoiceDueDate)) {
        $data['transaction']['due_dates'] = array(
                    'INVOICE' => date('Y-m-d', strtotime('+'.$this->novalnetinvoiceDueDate.' days'))
                );
        }

        $url = 'https://payport.novalnet.de/v2/seamless/payment';
        if ($this->novalnetinvoicePaymentAction == 'Authorize' && ($this->novalnetinvoiceOnHoldLimit <= $data['transaction']['amount'])) {
