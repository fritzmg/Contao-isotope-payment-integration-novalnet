...
class NovalnetInstalmentInvoice extends Postsale implements IsotopePayment
{
    /**
     * @var NovalnetHelper
     */
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        $data['transaction']['test_mode'] = !empty($this->novalnetinstalmentinvoiceTestMode) ? $this->novalnetinstalmentinvoiceTestMode : '0';
        ...
        $minAmount = $this->novalnetinstalmentinvoiceMinimumOrderAmount;
        ...
        if (empty($this->novalnetinstalmentinvoiceAllowB2B) && !empty($data['customer']['billing']['company'])) {
            unset($data['customer']['billing']['company']);
        }

        $url = 'https://payport.novalnet.de/v2/seamless/payment';
        if ($this->novalnetinstalmentinvoicePaymentAction == 'authorize' && ($this->novalnetinstalmentinvoiceOnHoldLimit <= $data['transaction']['amount'])) {
