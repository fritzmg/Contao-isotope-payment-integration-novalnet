...
class NovalnetPaypal extends Postsale implements IsotopePayment
{
    /**
     * @var NovalnetHelper
     */
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        $data['transaction']['test_mode'] = !empty($this->novalnetpaypalTestMode) ? $this->novalnetpaypalTestMode : '0';
        ...
        if ($this->novalnetpaypalOneClickShopping == 1) {
             $paymentID = $this->arrData['id'];

             $paymentData = \Database::getInstance()->execute("SELECT payment_data FROM tl_iso_product_collection WHERE member={$order->member} AND payment_id={$paymentID} AND payment_data!='' ORDER BY document_number DESC")
            ->fetchAllAssoc();
        ...
        if ($this->novalnetpaypalPaymentAction == 'authorize' && ($this->novalnetpaypalOnHoldLimit <= $data['transaction']['amount'])) {
