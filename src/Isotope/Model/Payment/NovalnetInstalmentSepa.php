...
class NovalnetInstalmentSepa extends Postsale implements IsotopePayment
{
    /**
     * @var NovalnetHelper
     */
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        $data['transaction']['test_mode'] = !empty($this->novalnetinstalmentsepaTestMode) ? $this->novalnetinstalmentsepaTestMode : '0';
        ...
        if ($this->novalnetinstalmentsepaOneClickShopping == 1) {             
        ...
        $minAmount = $this->novalnetinstalmentsepaMinimumOrderAmount;
        $minAmount = ($minAmount) ? $minAmount : 30000;
        ...
        if (empty($this->novalnetinstalmentsepaAllowB2B) && !empty($data['customer']['billing']['company'])) {
            unset($data['customer']['billing']['company']);
        }

        $url = 'https://payport.novalnet.de/v2/seamless/payment';
        if ($this->novalnetinstalmentsepaPaymentAction == 'authorize' && ($this->novalnetinstalmentsepaOnHoldLimit <= $data['transaction']['amount'])) {
