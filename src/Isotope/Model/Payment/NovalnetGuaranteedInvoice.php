...
class NovalnetGuaranteedInvoice extends Postsale implements IsotopePayment
{
    /**
     * @var NovalnetHelper
     */
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        $data['transaction']['test_mode'] = !empty($this->novalnetguaranteedinvoiceTestMode) ? $this->novalnetguaranteedinvoiceTestMode : '0';
        ...
        $minAmount = $this->novalnetguaranteedinvoiceMinimumOrderamount;
        ...
        if (empty($this->novalnetguaranteedinvoiceAllowB2B) && !empty($data['customer']['billing']['company'])) {
            unset($data['customer']['billing']['company']);
        }

        $url = 'https://payport.novalnet.de/v2/seamless/payment';
        if ($this->novalnetguaranteedinvoicePaymentAction == 'authorize' && ($this->novalnetguaranteedinvoiceOnHoldLimit <= $data['transaction']['amount'])) {
