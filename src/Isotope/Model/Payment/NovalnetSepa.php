...
class NovalnetSepa extends Postsale implements IsotopePayment
{
    /**
      * @var NovalnetHelper
      */
    ...
    */
    ...
    public function checkoutForm(IsotopeProductCollection $order, Module $module)
    {
        ...
        $data['transaction']['test_mode'] = !empty($this->novalnetsepaTestMode) ? $this->novalnetsepaTestMode : '0';
        ...
        if (!empty($this->novalnetsepaDueDate) && is_numeric($this->novalnetsepaDueDate)) {
            $duedate = date('Y-m-d', strtotime('+'.$this->novalnetsepaDueDate.' days'));
            $data['transaction']['due_dates'] = array( 'DIRECT_DEBIT_SEPA' => $duedate );
        }

        if ($this->novalnetsepaOneClickShopping == 1) {            
        ...
        if ($this->novalnetsepaPaymentAction == 'Authorize' && ($this->novalnetsepaOnHoldLimit <= $data['transaction']['amount'])) {
